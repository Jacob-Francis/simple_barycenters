...
for aprox_type in ['kl', 'tv']:
    ...
    for j, (d,k) in enumerate([(6,0), (6,1), (6,2), (6,3)]):
        ...
        barycentre = np.reshape(barycentre.detach().cpu().numpy(), (200,  200), order='F')
        img = ax.pcolormesh(
            X[:, :, 0],
            X[:, :, 1],
    ...
    fig_ex.savefig(f'spread_curves/grp3/grp3_bary_ex_rho1_{aprox_type}.png', bbox_inches='tight', dpi=200)
    plt.close()
    ...
    fig_ex.savefig(f'spread_curves/grp3/grp3_ellipsebary_ex_rho001_{aprox_type}.png', bbox_inches='tight', dpi=200)
