...
fig_mass, ax_mass = plt.subplots(1,1, figsize=(9, 7))
...
for data_set in [18, 19, 20]:
    ...
    for aprox_type in aprox_types:
        ...
        for k, t in enumerate(times):
            ...
            obs_mass = sum(obvs_dict[t]['observation'][0].flatten())/200**2
            mass_ratio[k] = barycentre.sum().item()/obs_mass
        max_val['TR'] = max(max_val['TR'], max(obs_se.max(), se_spread.max()))
        min_val['TR'] = min(min_val['TR'], min(obs_se.min(), se_spread.min()))

        # plot mass ratio
        mass_ratio_dict[aprox_type][data_set] = mass_ratio
...
fig_ss.savefig(f'spread_curves/grp9/grp9_ss_1820_rho{rho}.png', dpi=200, bbox_inches='tight')


# plotting mass ratio
for aprox_type in aprox_types:
    times = np.array(times)  
    x = np.arange(len(times))  # discrete positions
    ...
    for i, data_set in enumerate(datasets):
        ...
        ax_mass.bar(
            x + offset,
            mass_ratio_dict[aprox_type][data_set],
            width=bar_width,
            label=f"Dataset {data_set}",
            color=line_colors[data_set] 
        )

    ax_mass.set_xticks(x)
    ax_mass.set_xticklabels(times)
    ax_mass.set_xlabel("Time")
    ax_mass.set_ylabel("Mass ratio (barycentre / observation)")
    ax_mass.legend()
    ax_mass.grid(alpha=0.3)
    fig_mass.savefig(f'spread_curves/grp9/grp9_mass_{aprox_type}_{rho}.png', dpi=200, bbox_inches='tight')
...
fig_ss.savefig(f'spread_curves/grp9/grp9_ss_decomp_1820_rho{rho}.png', dpi=200, bbox_inches='tight')
