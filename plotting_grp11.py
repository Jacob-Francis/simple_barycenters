...
# fig_ex.savefig('/home/jjf817/PhD_jobs/simple_barycentres/spread_curves/grp11/grp11_ellipsebary_ex_rho1_kl.png', bbox_inches='tight', dpi=200)
    ...
    for aprox_type in aprox_types:
        ...
        for k, values in enumerate([se_spread, obs_se,  se_spread_decomp, obs_se_decomp]):
            if np.any(values <= 0):
                ...
                # flip sign
                assert np.mean(values[values <= 0]) < 1e-5
...
ax_ss[1].set(xscale='log', yscale='log')
...
fig_ss.savefig(f'spread_curves/grp11/grp11_ss_1114_rho{rho}.png', dpi=200, bbox_inches='tight')
...
ax_ss[0].set(xscale='log', yscale='log')
ax_ss[1].set(xscale='log', yscale='log')
...
fig_ss.savefig(f'spread_curves/grp11/grp11_ss_decomp_1114_rho{rho}.png', dpi=200, bbox_inches='tight')
...
for no, rho in enumerate([1.0, 0.01, 0.001]):
    ...
    for data_set in [23, 24, 25]:
        ...
        for aprox_type in aprox_types:
            ...
            # later
            # top right full se
            ax_ss[no+1].plot(se_spread, obs_se,  marker=markers[aprox_type], color=aprox_colors[aprox_type], linestyle=line_style[data_set], markersize=12, markerfacecolor='none')
...
for k, ax in enumerate(ax_ss.ravel()):
    ax.set(title=titles[k])
...
fig_ss.savefig(f'spread_curves/grp11/grp11_ss_2325_allrho.png', dpi=200, bbox_inches='tight')
