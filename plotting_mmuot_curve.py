...
# data_sets = [21] #  #[1, 2, 3, 4, 5, 6, 10, 12, 13, 14, 15, 16, 18, 19, 20, 21]  # [ 2, 5, 7, 8,9,  17, 11]
    ...
    for epsilon in epsilons:
        ...
        for l, (debiasing, rho) in enumerate([(True, 1.0), (True, 0.001), (False, 1.0), (False, 0.001)]):
            ...
            for aprox_type in aprox_types:
                ...
                se_spread_bias = np.zeros(len(times))
                ...
                for k, t in enumerate(times):
                    ...
                    temp = np.zeros(len(members)+1)
                    for i in range(len(members)+1):
                        temp[i] = bary_mmuot_cost[t][f'debias_{i}']
                    spread[k] = (bary_mmuot_cost[t]['bias'] - 1/(len(members)+1) * temp.sum()).item()

                    se_spread[k] = se_costs[k][0]['total_cost']
                    se_spread_bias[k] = sum(se_costs[k][0]['unbalanced_sinkhorn_terms'])
                ...
                ax[0, l].plot(times, spread, '--', marker=markers[aprox_type], markersize=10, label='spread', color=cost_colours['spread'])
                ax[0, l].plot(times, error, '-.', marker=markers[aprox_type], markersize=10, label='error', color=cost_colours['skill'])  
                ax[0, l].plot(times, se_spread, ':', marker=markers[aprox_type],  markersize=10, label='se spread', color=cost_colours['se_cost'])  
                ax[0, l].plot(times, se_spread_bias, '-', marker=markers[aprox_type],  markersize=10, label='se spread bias', color=cost_colours['se_cost_bias'])  

                # ax[0, l].plot(times, mass_ratio, '-', marker=markers[aprox_type], markerfacecolor='none', label='mass ratio', color=cost_colours['mass_ratio'])  
                ax[0,l].set_title(f"Av. mass ratio: {mass_ratio.mean():.4g}", fontsize=16)

                ax[1, l].plot(spread, error, 'k--', marker=markers[aprox_type], label=f'aprox={aprox_type}')
        ...
        try:
            plt.savefig(f'spread_curves/{data_set}/{data_set}_spread_eps{str(epsilon)[2:]}.png', dpi=200)
